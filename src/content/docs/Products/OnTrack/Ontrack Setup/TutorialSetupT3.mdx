...
If you have attempted the setup previously and encountered errors, it is recommended to restart Docker Desktop and follow this guide from the beginning to avoid configuration issues.
...
Note: The version number (`10.0.0-14`) comes from the `docker-compose.yaml` file in the `doubtfire-deploy` repository.
...
Git remotes allow you to sync with both your fork (`origin`) and the ThothTech repository (`upstream`).
