...
1. Install lcov via your package manager  
   e.g., `apt install lcov` (note that your Linux distrubtion may use a different package manager
   rather than `apt`, and you may need to `sudo` this command)
2. Go to the CMake Tools settings in VS Code, switch to the Workspace tab, and change the following
   settings:  
   **Coverage Info Files:**  
...
3. Rebuild the test project, then run tests with coverage with this button  
   ![Run Tests with Coverage](images/code-coverage/button-run-coverage.png)
4. After the tests run, the test explorer pane in VS Code should update with the coverage info:  
...
1. Install gcovr via homebrew `brew install gcovr`
2. Go to the CMake Tools settings in VS Code, switch to the Workspace tab, and change the following
   settings:  
   **Coverage Info Files:**  
   Click Add Item and add `${workSpaceFolder}/projects/cmake/build/lcov.info`  
