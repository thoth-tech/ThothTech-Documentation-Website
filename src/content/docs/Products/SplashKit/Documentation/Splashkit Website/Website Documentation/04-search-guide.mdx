...
While working on integrating DocSearch into the SplashKit website, I noticed
there was a bug regarding the `lvl0` heading where it was pulling too much data
causing the header to be a jumbled mess.

![Heading Bug](./images/heading-bug.png)

This is due to the `lvl0` searching for `span` tags that are contained within
the `summary` tags which is espcially for folders names used in the sidebar.
This can be navigated through the Editor tab and telling the `lvl0` variable to
only pull the first match per category.
...
By specifying the `.first()` function, it ensures it only returns the main
category title.
...
3. **Optional** Further​ configuration can be done by following guides on the
   <a
     href="https://starlight.astro.build/guides/site-search/#algolia-docsearch"
     target="_blank"
   >
