...
| POST /requirement                 | Add a new requirement.                      | Requirement  | TeachingPeriod |
...
| category              | String  | Category of the requirement (pre-requsuite etc.)     |
...
the requirement (complete 5 out of 21 units for example) will not be recorded. The primary aim is to
have the client side utilize the requirement data in conjunction with the units set on the course
map (and potentially units marked as complete) to provide visual indicators on the client side to
show requirement progression.
