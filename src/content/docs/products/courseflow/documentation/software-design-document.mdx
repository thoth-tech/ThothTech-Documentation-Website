...
backend APIs that need to be defined and what their role will be in supporting these features.
...
- A query system will need to be implemented where either drop down menus or a series of questions
  will be presented to the user to gauge what their interest areas are.
  - For the purposes of the MVP, interest areas will be manually set. Future iterations of this
    application can explore the use of Machine Learning to categorize units and courses based on
...
handled by the front end by using the returned data with the fields defined in the course map unit
...
Furthermore, to accommodate for the query system that will be used to categorize courses based on
...
| Field         | Type           | Description                                     |
| ------------- | -------------- | ----------------------------------------------- |
| courseId      | String         | MongoDB auto generated id.                      |
...
Please note, at this point in time, default course data will be added to the database. Further
considerations will be made once more options are available in handling of course data (separate
stream to add data such as an admin role access to CourseFlow, Deakin API access).
...
| Field         | Type           | Description                                     |
| ------------- | -------------- | ----------------------------------------------- |
| unitId        | String         | MongoDB auto generated id.                      |
...
Please note, at this point in time, default unit data will be added to the database. Further
considerations will be made once more options are available in handling of course data (separate
stream to add data such as an admin role access to CourseFlow, Deakin API access).

**User**

This will mainly be used to define users of the CourseFlow system so that we have a user id to match
with course map data. This will also be important for authentication and authorization (if needed)
when these features are introduced.

| Field       | Type   | Description                  |
| ----------- | ------ | ---------------------------- |
| userId      | String | MongoDB auto generated id.   |
...
| Endpoint            | Description                          | Request Body | Response Body |
| ------------------- | ------------------------------------ | ------------ | ------------- |
| GET /user           | Get all user data.                   | None         | User[]        |
| GET /user/username  | Get a user by username query param.  | None         | User[]        |
| POST /user          | Add a new user.                      | User         | User          |
| PUT /user/userId    | Updates an existing user via its id. | Unit         | User          |
| DELETE /user/userId | Deletes an existing user via its id. | None         | None          |
...
1. Gather course data (will focus on adding one course first which will likely be the Bachelor of
