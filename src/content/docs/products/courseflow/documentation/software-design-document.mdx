...
- A query system for interests will need to be implemented to gauge what a user's interest areas
  are.
  - For the purposes of the MVP, interest areas will be manually set when adding courses. Future
    iterations of this application can explore the use of Machine Learning to categorize units and
    courses based on their textual description.
...
interest area, this data also needs to include an array of strings to represent each interest areas.
For reference to ensure clarity, interest areas will include categories such as Application
Development, Game Development, Animation etc...
...
| interests    | String[]       | A list of interests to help group courses. |
...
| ------------------------ | -------------------------------------------------------- | ------------ | ------------- |
| GET /courses             | Get all course data.                                     | None         | Course[]      |
| GET /courses/courseId    | Get a course by id.                                      | None         | Course        |
| GET /courses/interests   | Get all course data that match with the interest params. | None         | Course[]      |
| POST /courses            | Add a new course.                                        | Course       | Course        |
| PUT /courses/courseId    | Updates an existing course via its id.                   | Course       | Course        |
| DELETE /courses/courseId | Deletes an existing course via its id.                   | None         | None          |
...
Additionally, units will also be categorized by interest areas.

The unit model will be as follows:
...
| interests    | String[]       | A list of interests to help group units. |
...
| GET /unit/unitId    | Get unit by id.                                       | None         | Unit          |
| GET /unit/interests | Get all unit data that match with the interest param. | None         | Unit[]        |
...
The user will play an important role in storing the user's respective course map and also, some
general information such as name, username and password. Outside of this, the user model will also
be required for authentication/authorization purposes once these features are introduced.

The data wil be modelled as shown in the table below:

| Field       | Type            | Description                  |
| ----------- | --------------- | ---------------------------- |
| userId      | Integer         | Auto generated id.           |
| userName    | String          | User's username.             |
| password    | String          | User's hashed password.      |
| name        | String          | User's name.                 |
| courseMap   | CourseMapUnit[] | Course map data.             |
| dateAdded   | Date            | Date this entry was added.   |
| dateUpdated | Date            | Date this entry was updated. |
...
trimester and a unit. To avoid nesting the data within these groups inside the database, an
alternative solution is to have a list of units and have fields for the respective year, trimester
and unit containers that will be populated on the client side web application.

As such, the CourseMapUnit structure can be modelled as shown below to achieve this result.
...
In the user data model, it simply needs to keep track of an array of this course map unit data.
Displaying the units correctly in their respective year, trimester and positional slot will be
handled by the front end by using the returned data.
...
To handle the specific requirements, we can model the data with the requirement type and the requirement definition.

As an example, for minimum credit points, the model will look like the table below.
...
In the course or units data, their list of requirements will then contain the above model as an entry with the
respective minimum credit points type and an integer defining the actual credit points requirement
(24 for example).

Another example includes pre-requisites, which will have a list of unit string ids, along with a requirement type that may be labelled as "pre-requisites".
