...
This section will provide a detailed analysis of the key requirements for the CourseFlow application to function correctly.
...
The handling of the course map context contributes to the key function of CourseFlow which is to allow a student to plan their course and see if said plan meets any requirements. As such, it is important that the course map has the structural capabilities to accommodate slots for multiple years, trimesters and units to cater for part-time and full-time students.
...
The requirements data forms a crucial aspect of this application as it defines requirements/constraints for courses and units that will be utilized to indicate to the user if their course map is valid or not.

**Course**

Course data formulates an important aspect of this application for the following reasons:

- Students need to select a course from a list of courses (ideally categorized properties such as discipline area or school).
- The course map will need a course to be set in order to display the correct information in relation to course requirements.
...
The data associated with courses in the context of CourseFlow will have some minimum requirements that will be needed so that it can be presented to the user on the client side. 

Most importantly, course requirements (such as minimum credit points, core units etc.) will also need to be captured in each course.
...
Similar to the course data, unit data will also be important for course map planning and ensuring unit requirements are correctly presented to students.
...
Much like the course data, an important aspect of the data that needs to be captured by the unit is the requirements. Requirements can include specific pre-requisite units, co-requisite units and minimum number of units belonging to a specific Deakin University school (SIT etc.) to be completed.
