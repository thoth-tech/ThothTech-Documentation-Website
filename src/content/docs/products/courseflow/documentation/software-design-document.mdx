...
- There are two key parts to this feature:
  - Core units of any kind will be shown to the user in an easy to access manner. This can be
    through a course based template or through a window with core units already present and ready to
    be drag and dropped.
...
| Endpoint             | Description                                           | Request Body | Response Body |
| -------------------- | ----------------------------------------------------- | ------------ | ------------- |
| GET /units           | Get all unit data.                                    | None         | Unit[]        |
| GET /units/unitId    | Get unit by id.                                       | None         | Unit          |
| GET /units/interests | Get all unit data that match with the interest param. | None         | Unit[]        |
| POST /units          | Add a new unit.                                       | Unit         | Unit          |
| PUT /units/unitId    | Updates an existing unit via its id.                  | Unit         | Unit          |
| DELETE /units/unitId | Deletes an existing unit via its id.                  | None         | None          |

Please note, at this point in time, default unit data will be added to the database. Further
considerations will be made once more options are available in handling of unit data (separate
stream to add data such as an admin role access to CourseFlow, Deakin API access).
