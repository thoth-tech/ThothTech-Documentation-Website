...
1. **Choose a Planner Card or Idea**:  
   Pick an existing planner card that matches a SplashKit function, or create a unique example. Make sure the idea clearly demonstrates the function in a practical or visually interesting way.
...
3. **Write the Program in Multiple Languages**:  
   ...
   - **C#**: Provide both a version using top-level statements for simplicity and an Object-Oriented (OOP) version for a more structured approach.
...
4. **Capture Output Screenshots and Write Explanations**:  
   Run each program version, take screenshots of the output, and write a brief explanation for each language version. Describe what the program does, which function it demonstrates, and highlight any important details.

5. **Add Files to the Correct Folder in the SplashKit Repository**:  
   Organize your files in the appropriate folders within the SplashKit repository. Ensure file names and directory paths are consistent with SplashKit’s structure.

6. **Submit a Pull Request for Usage Examples**:  
   Follow the steps in the [Pull Requests for Usage Examples guide](/products/splashkit/splashkit-website/usage-examples/03-usage-pull-request). Use the provided PR template, describe your example clearly, and attach explanations and screenshots to the pull request.
...
<CardGrid>
  <LinkCard
    ...
    description="How to create usage examples for the Splashkit website, including the necessary steps and guidelines."
