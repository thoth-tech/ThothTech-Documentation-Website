...
The goal is to create **one usage example per function** in the SplashKit library. Building out these
single examples for each function is the priority to round out the variety of examples available.

If you come up with a creative idea for a function that already has an example, please share it in
the SplashKit group chat or reach out to your Capstone Mentor to discuss whether it's a good
...
Now that your files are named, add all of your files to the `usage-examples-files` folder under the
appropriate function category (e.g., `terminal/write_line/`). The location in the Statlight.io repo is:
...
details on proper file naming and placement. This is also found within the repo in the `CONTRIBUTING.md`.
