...
sidebar:
  label: Key Website Development Files
  order: 3
---
...
5. [Peer Reviews: What to Look For](#peer-reviews-what-to-look-for)
   ...
   - [.jsx and .tsx Files](#jsx-and-tsx-files-1)
...
4.  **Page Transitions and Animations**
...
Example: Editing `Sidebar.astro` to update navigation links
...
- **What to update**:
  - If adding or editing interactive elements like carousels, forms, or sliders, modify these files.
    They’re typically located in `src/components/react/`.
...
Example: Adjusting the colour palette in `custom.css`
...
Example: Adding custom styles for a specific component
...
- An image used in a physics tutorial stored in `src/content/docs/guides/physics/images` should sit
  alongside the tutorial file in that same folder.
